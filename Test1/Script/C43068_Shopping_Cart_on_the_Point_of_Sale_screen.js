...
function C43068_Shopping_Cart_on_the_Point_of_Sale_screen()
{ 
  ...
  var givenPaymentType ="Cash";
   try {
    Log.AppendFolder("C43068_Shopping_Cart_on_the_Point_of_Sale_screen");
    ...
    if(childCount == 2){
     ...
     Button.clickOnButton(deleteImageGroup1); 
        if(alertCancelReservation.Exist && alertCancelReservation.VisibleOnScreen){
            Button.clickOnButton(alertCancelReservationYes);            
        }else{
            merlinLogError("Alert is not displayed.")
        ...
        cartaddedItemList.Refresh();
        aqUtils.Delay(1000);
        childCount =cartaddedItemList.ChildCount;            
